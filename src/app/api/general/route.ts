...
export async function POST(request: Request) {
    try {
        ...
        // レコードをバッチで取得
        let generalResponses: GeneralResponse[] = [];
        let currentSkip = 0;
        ...
        while (moreDataToFetch) {
            ...
            if (batch.length > 0) {
                generalResponses = generalResponses.concat(batch);
        ...
        return NextResponse.json(generalResponses);
    } catch (error: unknown) {
        console.error('Error in General API route:', error);
