...
export async function GET(
    ...
    try {
        ...
        const urges = searchParams.getAll('urge');
        ...
        if (urges.length > 0) {whereConditions.push({OR: urges.map(urge => ({urge: urge}))});}
        ...
        if (action === 'getInfo') {
            // ページネーションの情報取得
            const conut: number = await prisma.elois.count({where: {AND: whereConditions}});

            const categoriesInfo: Category[] = [{id: 'elois', name: 'Eロイス', count: conut}];
            ...
            let responses: EloisResponse[] = [];
            let currentSkip = 0;
            let moreDataToFetch = true;
            while (moreDataToFetch) {
                const batch: EloisResponse[] = await prisma.elois.findMany({
                    where: {
                        AND: whereConditions,
                    },
                    include: {
            ...
            const records = responses.map(response => parseElois(response));
            const dataForPage: Category[] = [{id: 'dlois', name: 'Eロイス', count: records.length, records: records}];
