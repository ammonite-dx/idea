...
export async function GET(
    ...
    const ref_power: Power|null = ref_power_id ? (await (await fetch(`/api/power?id=${ref_power_id}`,{method:"GET"})).json()) as Power : null;
...
export async function POST(
    ...
    const searchCondition = {
        ...
        const dloises: { [key: string]: Dlois[] } = { "Dロイス": (await Promise.all(searchResults.map(async ({id}: {id:string}) => (await (await fetch(`/api/dlois?id=${id}`,{method:"GET"})).json()) as Dlois))) };
