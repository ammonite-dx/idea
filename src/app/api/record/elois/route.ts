...
export async function POST(
    ...
    const searchCondition = {
        ...
        const eloises: { [key: string]: Elois[] } = { "Eロイス": (await Promise.all(searchResults.map(async ({id}: {id:string}) => (await (await fetch(`/api/record/elois?id=${id}`,{method:"GET"})).json()) as Elois))) };
