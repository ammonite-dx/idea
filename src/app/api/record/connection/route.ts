...
export async function POST(
    ...
    const connections: { [key: string]: Connection[] } = Object.fromEntries(await Promise.all(toArray(searchParams["category"], ITEM_CATEGORIES).map(async category => {
        ...
        const connectionsInCategory = await Promise.all(searchResultsInCategory.map(async ({id}: {id:string}) => (await (await fetch(`/api/record/connection?id=${id}`,{method:"GET"})).json()) as Connection));
