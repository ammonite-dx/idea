...
export async function GET(
    ...
    const ref_armor: Armor|null = ref_armor_id ? (await (await fetch(`/api/record/armor?id=${ref_armor_id}`,{method:"GET"})).json()) as Armor : null;
...
export async function POST(
    ...
    const powers: {[key: string]: Power[]} = Object.fromEntries(await Promise.all(toArray(searchParams["category"], POWER_CATEGORIES).map(async category => {
        ...
        const powersInCategory = await Promise.all(searchResultsInCategory.map(async ({id}: {id:string}) => (await (await fetch(`/api/record/power?id=${id}`,{method:"GET"})).json()) as Power));
