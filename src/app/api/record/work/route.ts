...
export async function POST(
    ...
    const searchCondition = {
        ...
        const works: { [key: string]: Work[] } = { "ワークス": (await Promise.all(searchResults.map(async ({id}: {id:string}) => (await (await fetch(`/api/record/work?id=${id}`,{method:"GET"})).json()) as Work))) };
