...
export async function GET(
    ...
    if (!searchResult) return NextResponse.json({ error: "General item not found" }, { status: 404 });
...
export async function POST(
    ...
    const generals: { [key: string]: General[] } = Object.fromEntries(await Promise.all(toArray(searchParams["category"], ITEM_CATEGORIES).map(async category => {
        ...
        const generalsInCategory = await Promise.all(searchResultsInCategory.map(async ({id}: {id:string}) => (await (await fetch(`/api/record/general?id=${id}`,{method:"GET"})).json()) as General));
