...
export async function POST(
    ...
    const vehicles: { [key: string]: Vehicle[] } = Object.fromEntries(await Promise.all(toArray(searchParams["category"], ITEM_CATEGORIES).map(async category => {
        ...
        const vehiclesInCategory = await Promise.all(searchResultsInCategory.map(async ({id}: {id:string}) => (await (await fetch(`/api/vehicle?id=${id}`,{method:"GET"})).json()) as Vehicle));
