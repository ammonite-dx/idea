...
export default async function searchRecords<K extends keyof TypeMap>(
  ...
  if (kind === "item") {
    switch (toString(searchParams["item-type"], "指定なし")) {
      ...
      default:
        const weapons = (await (await fetch(`/api/weapon`,{method:"POST", body:JSON.stringify(searchParams)})).json());
        const armors = (await (await fetch(`/api/armor`,{method:"POST", body:JSON.stringify(searchParams)})).json());
        const vehicles = (await (await fetch(`/api/vehicle`,{method:"POST", body:JSON.stringify(searchParams)})).json());
        const connections = (await (await fetch(`/api/connection`,{method:"POST", body:JSON.stringify(searchParams)})).json());
        const generals = (await (await fetch(`/api/general`,{method:"POST", body:JSON.stringify(searchParams)})).json());
        ...
        return items as { [key: string]: TypeMap[K][] };
    }
  } else {
